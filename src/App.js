...
const MovieForm = (props) => {
  ...
  const [imdbVotes, setimdbVotes] = useState(0);
...
const DropItem = (props) => {
  ...
  return(
    <div className="dropItem" onClick={()=> setOpen(!open)} >
...
const MovieItem = (props) => {
  return (
    <div key={props.index}>
      ...
      <p>imdb Votes: {props.movie.imdbVotes}</p>
      <p>imdb ID: {props.movie.imdbID}</p>
      <p>Type: {props.movie.type}</p>
      <p>Response: {props.movie.response}</p>
      <img src="">Image: {props.movie.response}</img>
    </div>
  );
};
...
// const [movieImage, setMovie] = useState(sampleMovies[0].Images);
...
{
  /* <div>        
<img src={props.moviePic[movieIndex].Images[0]}></img>
<br></br>
<button onClick={()=>{
  if(movieIndex < 1){
    setMovieIndex(0)
...
{
  /* <h1 style={{color: color}}>Colors: {colors.toString()}</h1>
      ...
      }}>Add Color Button</button>
      <button onClick={()=>{
        setColors([])
      }}>Remove All Colors</button>   */
}
...
//   return(
//     <div>
//       <h1>Blog Post Title: {props.blog.title}</h1>
//     </div>
